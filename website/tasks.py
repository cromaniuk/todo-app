...
def update_task(task_id):
        ...
        if existing_task:
                if 'title' not in request.json:
                        return jsonify({"error": "Missing title value"}), 403, {'Content-Type': 'application/json'}
                if 'description' not in request.json:
                        return jsonify({"error": "Missing description value"}), 403, {'Content-Type': 'application/json'}
                if 'completed' not in request.json:
                        return jsonify({"error": "Missing completed value"}), 403, {'Content-Type': 'application/json'}
                ...
                return jsonify({"error": f"Task with ID {task_id} not found"}), 404, {'Content-Type': 'application/json'}
...
def delete_task(task_id):
        ...
        if existing_task:
                ...
                db.session.commit()
                return jsonify({"message": f"Task with ID {task_id} successfully deleted"}), 201, {'Content-Type': 'application/json'}
        else:
                return jsonify({"error": f"Task with ID {task_id} not found"}), 404, {'Content-Type': 'application/json'}
