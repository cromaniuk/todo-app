...
def test_login_missing_username(client, test_user):
    payload = json.dumps({"password": test_user.password})
    response = client.post(
        "/login", headers={"Content-Type": "application/json"}, data=payload
    )
...
def test_login_missing_password(client, test_user):
    payload = json.dumps({"username": test_user.username})
    response = client.post(
        "/login", headers={"Content-Type": "application/json"}, data=payload
    )
    assert response.status_code == 403
...
def test_login_invalid_password(client, test_user):
    payload = json.dumps(
        {"username": test_user.username, "password": "incorrectpassword"}
    )
    response = client.post(
        "/login", headers={"Content-Type": "application/json"}, data=payload
    )
...
def test_successful_registration(client, new_user):
    payload = json.dumps({"username": new_user.username, "password": new_user.password})
    response = client.post(
